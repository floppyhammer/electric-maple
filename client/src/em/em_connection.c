...
static void
em_conn_webrtc_on_prepare_data_channel(GstElement *webrtcbin,
...
em_connection_set_pipeline(EmConnection *em_conn, GstPipeline *pipeline)
{
	...
	g_signal_connect(em_conn->webrtcbin, "prepare-data-channel", G_CALLBACK(em_conn_webrtc_on_prepare_data_channel),
	                 NULL);
#endif
}
