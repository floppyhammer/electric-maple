...
bool GLSwapchain::enumerateAndGenerateFramebuffers(XrSwapchain swapchain) {
    ...
    XrResult result = xrEnumerateSwapchainImages(swapchain,
                                                 countOutput,
                                                 &countOutput,
    ...
    //            // Return back an array of pointers to each swapchain image struct so the consumer doesn't need to
    //            know the type/size. std::vector<XrSwapchainImageOpenGLESKHR> swapchainImageBuffer(countOutput,
    //            {XR_TYPE_SWAPCHAIN_IMAGE_OPENGL_ES_KHR}); std::vector<XrSwapchainImageBaseHeader*> swapchainImageBase;
    ...
    for (GLsizei i = 0; i < n; ++i) {
        ALOGI("%s: Index %d: Binding framebuffer name %d to texture ID %d",
              __FUNCTION__,
              i,
              framebuffers_[i],
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, swapchainImages_[i].image, 0);
