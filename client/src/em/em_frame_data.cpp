...
void FrameDataAccumulator::recordDecodeTime(int64_t frameId, int64_t decodeTime) {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_accum.addDataFor(frameId, FrameData{decodeTime});
}

void FrameDataAccumulator::recordDisplayTime(int64_t frameId, int64_t displayTime) {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_accum.updateDataFor(frameId, [=](FrameData &data) { data.displayTime = displayTime; });
}

void FrameDataAccumulator::emitCompleteRecords(PfnEmitUpMessage pfn, void *userdata) {
    std::lock_guard<std::mutex> lock(m_mutex);
