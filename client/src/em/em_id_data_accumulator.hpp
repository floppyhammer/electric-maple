...
public:
    ...
    ValueType *getForId(IdType id);

    /// Get a pointer to the const value corresponding to that ID, or nullptr if not found.
    ValueType const *getConstForId(IdType id) const;

    /// Get a pointer to the const value corresponding to that ID, or nullptr if not found.
    ValueType const *getForId(IdType id) const;
     ...
     */
    bool addDataFor(IdType id, ValueType &&value);
     ...
     */
    template <typename F>
    bool updateDataFor(IdType id, F &&dataUpdater) {
     ...
     */
    template <typename F>
    bool constVisitAll(F &&dataHandler) const {
        bool haveAny = false;
...
private:
    ...
    void minMaxMatch(IdType id, ArrayIterator &minElt, ArrayIterator &maxElt, ArrayIterator &matchingElt) {
    ...
    static bool isPairPopulated(PairType const &p) {
        return p.first != kSentinel;
    }
    static void markPairUnpopulated(PairType &p) {
        p.first = kSentinel;
        p.second = {};
    }
    ArrayType m_data;
...
template <typename ValueType, std::size_t MaxSize>
inline bool IdDataAccumulator<ValueType, MaxSize>::addDataFor(IdType id, ValueType &&value) {
    if (id == kSentinel) {
...
template <typename ValueType, std::size_t MaxSize>
inline void IdDataAccumulator<ValueType, MaxSize>::clear() {
    for (auto &p : m_data) {
        markPairUnpopulated(p);
    }
}
...
template <typename ValueType, std::size_t MaxSize>
inline ValueType *IdDataAccumulator<ValueType, MaxSize>::getForId(IdType id) {
    const auto b = m_data.begin();
    const auto e = m_data.end();
...
template <typename ValueType, std::size_t MaxSize>
inline ValueType const *IdDataAccumulator<ValueType, MaxSize>::getConstForId(IdType id) const {
    const auto b = m_data.begin();
    const auto e = m_data.end();
...
template <typename ValueType, std::size_t MaxSize>
inline ValueType const *IdDataAccumulator<ValueType, MaxSize>::getForId(IdType id) const {
    return getConstForId(id);
}

template <typename ValueType, std::size_t MaxSize>
inline size_t IdDataAccumulator<ValueType, MaxSize>::size() const {
