...
em_remote_experience_report_pose(EmRemoteExperience *exp, XrTime predictedDisplayTime, InputState &inputState)
{
...

	{
		...
		result = xrLocateSpace(inputState.handAimSpace[Side::LEFT], exp->xr_owned.worldSpace,
...

	{
		...
		handLocalLocation.type = XR_TYPE_SPACE_LOCATION;
		handLocalLocation.next = NULL;
		result = xrLocateSpace(inputState.handAimSpace[Side::RIGHT], exp->xr_owned.worldSpace,
		                       predictedDisplayTime, &handLocalLocation);
		if (result != XR_SUCCESS) {
...
em_remote_experience_poll_and_render_frame(EmRemoteExperience *exp, InputState &inputState)
{
	...
	// The predicted display time is used to calculate the predicted head pose (the position and orientation of the
	// user's head at that future moment)
                                                 ...
                                                 XrCompositionLayerProjectionView *projectionViews)
{
	...
	std::chrono::duration<double, std::milli> waitDurationMs =
