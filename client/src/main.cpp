...
void initialize_handtracking(struct em_state &state) {
    PFN_xrCreateHandTrackerEXT pfnXrCreateHandTrackerEXT = nullptr;
    CheckXrResult(xrGetInstanceProcAddr(state.instance,
...
void initialize_actions(struct em_state &state) {
    ...
    CheckXrResult(xrStringToPath(state.instance, "/user/hand/left/input/squeeze/value", &squeezeValuePath[Side::LEFT]));
    CheckXrResult(
        xrStringToPath(state.instance, "/user/hand/right/input/squeeze/value", &squeezeValuePath[Side::RIGHT]));
    CheckXrResult(xrStringToPath(state.instance, "/user/hand/left/input/squeeze/force", &squeezeForcePath[Side::LEFT]));
    CheckXrResult(
    ...
    CheckXrResult(xrStringToPath(state.instance, "/user/hand/left/input/trigger/value", &triggerValuePath[Side::LEFT]));
    CheckXrResult(
    ...
    // Suggest bindings for KHR Simple.
    {
        ...
        CheckXrResult(xrStringToPath(state.instance,
        ...
        if (buffer.type == XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED) {
            ...
            switch (event->state) {
                case XR_SESSION_STATE_IDLE:
                    ALOGI("OpenXR session is now IDLE");
                    break;
                ...
                case XR_SESSION_STATE_SYNCHRONIZED:
                    ALOGI("OpenXR session is now SYNCHRONIZED");
                    break;
                case XR_SESSION_STATE_VISIBLE:
                    ALOGI("OpenXR session is now VISIBLE");
                    break;
                case XR_SESSION_STATE_FOCUSED:
                    ALOGI("OpenXR session is now FOCUSED");
                    break;
...
void android_main(struct android_app *app) {
    ...
    XrSystemGetInfo systemInfo = {.type = XR_TYPE_SYSTEM_GET_INFO, .formFactor = XR_FORM_FACTOR_HEAD_MOUNTED_DISPLAY};
    ...
    result =
        xrEnumerateViewConfigurations(_state.instance, _state.system, 2, &viewConfigurationCount, viewConfigurations);
    ...
    xrEnumerateViewConfigurationViews(_state.instance,
                                      _state.system,
                                      XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO,
                                      0,
                                      &viewCount,
                                      NULL);
    result = xrEnumerateViewConfigurationViews(_state.instance,
                                               _state.system,
                                               XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO,
                                               2,
                                               &viewCount,
    ...
    XrSessionCreateInfo sessionInfo = {.type = XR_TYPE_SESSION_CREATE_INFO,
