                     ...
                     bool flip_y)
{
	...
	int64_t system_clock = os_monotonic_get_ns();
	int64_t pipeline_clock = ems_gstreamer_pipeline_get_current_time(c->gstreamer_pipeline);
...
static xrt_result_t
ems_compositor_request_display_refresh_rate(struct xrt_compositor *xc, float display_refresh_rate_hz)
{
	// Do nothing
	return XRT_SUCCESS;
}
