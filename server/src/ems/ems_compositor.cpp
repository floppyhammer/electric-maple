...
static struct vk_bundle *get_vk(struct ems_compositor *c) {
    return &c->base.vk;
}
...
static VkResult select_instances_extensions(struct ems_compositor *c,
                                            struct u_string_list *required,
                                            struct u_string_list *optional) {
...
static bool compositor_init_vulkan(struct ems_compositor *c) {
    ...
    if (ret != VK_SUCCESS) {
        VK_ERROR(vk, "select_instances_extensions: %s\n\tFailed to select instance extensions.", vk_result_string(ret));
...
static bool compositor_init_pacing(struct ems_compositor *c) {
...
static bool compositor_init_info(struct ems_compositor *c) {
...
static bool compositor_init_sys_info(struct ems_compositor *c, struct xrt_device *xdev) {
...
static inline em_proto_Pose to_proto(const struct xrt_pose &pose) {
...
void pack_blit_and_encode(struct ems_compositor *c,
    ...
    msg.frame_data.has_P_localSpace_viewSpace = true;
    msg.frame_data.P_localSpace_viewSpace = to_proto(lvd->pose);
    ...
    // Commented to fix crash on Android
...
static xrt_result_t ems_compositor_begin_session(struct xrt_compositor *xc, const struct xrt_begin_session_info *info) {
...
static xrt_result_t ems_compositor_end_session(struct xrt_compositor *xc) {
    struct ems_compositor *c = ems_compositor(xc);
...
static xrt_result_t ems_compositor_predict_frame(struct xrt_compositor *xc,
...
static xrt_result_t ems_compositor_mark_frame(struct xrt_compositor *xc,
                                              int64_t frame_id,
...
static xrt_result_t ems_compositor_begin_frame(struct xrt_compositor *xc, int64_t frame_id) {
    struct ems_compositor *c = ems_compositor(xc);
...
static xrt_result_t ems_compositor_discard_frame(struct xrt_compositor *xc, int64_t frame_id) {
    struct ems_compositor *c = ems_compositor(xc);
...
static xrt_result_t ems_compositor_layer_commit(struct xrt_compositor *xc, xrt_graphics_sync_handle_t sync_handle) {
    COMP_TRACE_MARKER();
    ...
    for (uint32_t i = 0; i < c->base.layer_accum.layer_count; i++) {
        ...
        switch (layer.data.type) {
            ...
            default:
                U_LOG_E("Unhandled layer type %d", layer.data.type);
                break;
        }
    }
...
static xrt_result_t ems_compositor_get_swapchain_create_properties(struct xrt_compositor *xc,
