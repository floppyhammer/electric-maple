...
static void
ems_signaling_server_handle_text_message(EmsSignalingServer *server,
...
static void
ws_message_cb(SoupWebsocketConnection *connection, gint type, GBytes *message, gpointer user_data)
{
	switch (type) {
...
static void
ws_closed_cb(SoupWebsocketConnection *connection, gpointer user_data)
{
...
static void
ems_signaling_server_add_websocket_connection(EmsSignalingServer *server,
                                              SoupWebsocketConnection *connection,
...
ems_signaling_server_class_init(EmsSignalingServerClass *klass)
{
	...
	signals[SIGNAL_UP_MESSAGE] = g_signal_new("up_message", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_LAST, 0, NULL,
