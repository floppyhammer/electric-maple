...
// #define WEBSOCKET_URI_DEFAULT "ws://10.11.9.31:52356/ws"
...
static void data_channel_error_cb(GstWebRTCDataChannel *datachannel, void *data) {
    U_LOG_E("Error");
    abort();
}

static void data_channel_close_cb(GstWebRTCDataChannel *datachannel, gpointer timeout_src_id) {
...
static void data_channel_message_data_cb(GstWebRTCDataChannel *datachannel, GBytes *data, void *user_data) {
    U_LOG_I("Received data channel message data: %u", (uint32_t)g_bytes_get_size(data));
}

static void data_channel_message_string_cb(GstWebRTCDataChannel *datachannel, gchar *str, void *user_data) {
    U_LOG_I("Received data channel message string: %s", str);
}

static gboolean datachannel_send_message(gpointer unused) {
    g_signal_emit_by_name(datachannel, "send-string", "Hi! from EMS test client");

    return G_SOURCE_CONTINUE;
}

static void webrtc_on_data_channel_cb(GstElement *webrtcbin, GstWebRTCDataChannel *data_channel, void *user_data) {
